...
grid_str = '64_cube';
%Np_str =  '6.5e5'
%coarse_dt = [0.4 0.2 0.1 0.05 0.025 0.0125 0.00625 0.003125];
...
for t=1:length(coarse_dt) 
    ...
    for r=1:nranks
        for nc = 1:nCycles
            ...
            %end
            nCyc = 1;
            %file = [dir, num2str(nCyc),'_cycles/',num2str(sranks),'x',num2str(nranks),'/coarse_PIC/coarse_dt_',num2str(coarse_dt(t)),'/data/localError_rank_', num2str(r-1),'_nc_',num2str(nc),'.csv'];
            file = [dir, num2str(nCyc),'_cycles/',num2str(sranks),'x',num2str(nranks),'/coarse_PIF/coarse_tol_0.00001/coarse_dt_',num2str(coarse_dt(t)),'/data/localError_rank_', num2str(r-1),'_nc_',num2str(nc),'.csv'];
    ...
    %if PLOT_MAX_ERROR
        ...
        pt(ct) = semilogy((1:max_iter)', maxRerror(1:max_iter,t),'LineStyle','-','Marker','*','Color',color_map(mod(t,7)+1,:),'LineWidth',1.5);
        hold on;
        semilogy((1:max_iter)', maxPerror(1:max_iter,t),'LineStyle','-','Marker','s','Color',color_map(mod(t,7)+1,:),'LineWidth',1.5);
        % ylim([1e-6 1])
        hold on;
        semilogy((1:max_iter)', Referrorit(:),'LineStyle','--','Marker','d','Color',color_map(mod(t,7)+1,:),'LineWidth',1.5);
...
%xlim([1 8]);
set(gca,'Fontsize',14);
%legend([pt],'$\Delta t  = 0.4$','$\Delta t  = 0.2$','$\Delta t  = 0.1$','$\Delta t  = 0.05$','$\Delta t  = 0.025$','$\Delta t  = 0.0125$','$\Delta t  = 0.00625$','$\Delta t  = 0.003125$','Location','northeast','FontSize',12);  
legend([pt],'$\Delta t  = 0.05$','$\Delta t  = 0.025$','$\Delta t  = 0.0125$','$\Delta t  = 0.00625$','$\Delta t  = 0.003125$','Location','northeast','FontSize',12);  
%legend('Pos., $\epsilon=10^{-3}$','Vel., $\epsilon=10^{-3}$','Pos., $\epsilon=10^{-4}$','Vel., $\epsilon=10^{-4}$','Pos., $\epsilon=10^{-5}$','Vel., $\epsilon=10^{-5}$',...
%'Pos., $\epsilon=10^{-6}$','Vel., $\epsilon=10^{-6}$','Pos., $\epsilon=10^{-7}$','Vel., $\epsilon=10^{-7}$','FontSize',12,'Location','northeast','Box','off');
%title([num2str(allCycles(iCycles)),'  ', test_str]);
%if EXPORT_GRAPHICS
exportgraphics(figmax,[test_str,'_MaxlocalError_Vs_Iter_',grid_str,'_',Np_str,'_CIC_all_coarse_dt_nranks_',num2str(nranks),'_ncycles_', num2str(nCycles), '.pdf']);


%iterCons = [1 2 3 4 5 6 7];
iterCons = [1 2 3 4 5 6];
fig = figure;
for it=1:length(iterCons)
    loglog(coarse_dt(:), maxRerror(iterCons(it),:)','LineStyle','-','Marker','*','Color',color_map(it,:),'LineWidth',1.5);
    hold on;
    %loglog(coarse_dt(:), maxPerror(iterCons(it),:)','LineStyle','-','Marker','s','Color',color_map(it,:),'LineWidth',1.5);
    %hold on;
    Referror = zeros(length(coarse_dt),1);
    %Referror(1) = maxRerror(iterCons(it),1);
    %Referror(2:end) = maxRerror(iterCons(it),1) ./ (((coarse_dt(1) ./ coarse_dt(2:end)).^2).^iterCons(it));
    Referror(:) = (coarse_dt(:).^2).^iterCons(it);
    %Referror(:) = ((nufft_tol(:)).^(iterCons(it))) ./ factorial(iterCons(it));
...
%legend('Pos. error','Vel. error','$Ngrid^{-3.0}$','Location','best','FontSize',12);
    legend('$k=1$','$\Delta t^2$','$k=2$','$\Delta t^4$','$k=3$','$\Delta t^6$','Location','bestoutside','FontSize',12);
legend('boxoff');
...
exportgraphics(fig,[test_str,'_MaxlocalError_Vs_coarse_dt_',grid_str,'_',Np_str,'_nranks_',num2str(nranks),'_ncycles_', num2str(nCycles),'.pdf']);
